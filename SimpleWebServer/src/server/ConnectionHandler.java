...
public class ConnectionHandler implements Runnable {
	...
	public ConnectionHandler(ConnectionManager manager, Server server, Socket socket) {
		this.cManager = manager;
		this.server = server;
		this.socket = socket;
		if (setupStreams()) {
			this.reqManager = new RequestManager(this, this.inStream);
			this.resManager = new ResponseManager(server, this.outStream);
		}
	}
	...
	public void run() {
		...
		while (!this.reqManager.isOverloaded()) {
			...
			} else if (!this.reqManager.isServicable()) {
				//TODO: Fix "magic number" 503.
				this.resManager.addBadResponse(503);
			} else {
